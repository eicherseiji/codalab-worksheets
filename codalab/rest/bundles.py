...
def _update_bundle_contents_blob(uuid):
    ...
    - `state_on_success`: (optional) Update the bundle state to this state if
      ...
      Default is 'ready'.
    - `notify_worker_id_on_success`: (optional). If set, this endpoint runs
      asynchronously -- it runs upload_to_bundle_store in a separate thread,
      then notifies the given worker id with the `mark_upload_results_finished`
      message once the upload is done. This parameter should be set when workers
      are uploading results, to ensure HTTP connections remain short and don't get
      easily dropped. Default is None (in which case, the endpoint is synchronous).
    """
    ...
    notify_worker_id_on_success = request.query.get('notify_worker_id_on_success', default=None)
    ...
    def do_upload():
        ...
        try:
            ...
            if notify_worker_id_on_success is not None:
                local.worker_model.send_json_message(
                    notify_worker_id_on_success, {'type': 'mark_finalized', 'uuid': bundle.uuid}, 0.2
                )
    ...
    if notify_worker_id_on_success is None:
        # No threads, synchronous
        do_upload()
    else:
        t = Thread(target=do_upload, args=())
        t.start()
